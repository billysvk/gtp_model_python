...
#               steps=2, batch_size=1)   # steps is max number of training steps
# single_text = gpt2.generate(sess, return_as_list=True)[0]
# print(single_text)
# gpt2.generate(sess)        
# Note: Saving the fine-tuned model using gpt2.save_gpt2() is optional, 
# but it allows you to reuse the fine-tuned model later without retraining.  
...
def generate_response(prompt):
    ...
    # Adjust the temperature parameter to control the randomness of the generated responses.
    #  Higher values (e.g., 1.0) make responses more diverse but potentially less coherent,
    #  while lower values (e.g., 0.5) produce more focused but repetitive responses.
    response = gpt2.generate(sess, prefix=prompt, temperature=1.0, return_as_list=True)[0]
